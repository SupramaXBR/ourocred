...
if (isset($resposta['IDECLI'], $resposta['TOKEN'], $_SESSION['token']) && $_SESSION['token'] === $resposta['TOKEN']) {
   ...
   try {
      ...
      $sql = "UPDATE clientes_cpl 
              SET STAAPV = 'V', DTAALT = NOW() 
              WHERE IDECLI = :idecli";
      ...
      if (!$stmt->execute()) {
         http_response_code(500);
         echo json_encode(['mensagem' => 'Erro ao aprovar documentos no banco de dados']);
         exit;
      }

      http_response_code(200);
      echo json_encode(['mensagem' => 'Documentos aprovados com sucesso!']);
