...
if (isset($resposta['CODREG'], $resposta['TOKEN'], $_SESSION['token']) && $_SESSION['token'] === $resposta['TOKEN']) {
   $vsIdeMov = $resposta['IDEMOV'];
   $vsCodReg = $resposta['CODREG'];
   $vsUsrAlt = 'User';

   try {
      $sql = "UPDATE clientes_saque 
                SET STASAQ = 'C', USRALT = :usralt, DTAALT = NOW()
                WHERE CODREG = :codreg";
      ...
      $sql_saldo = "UPDATE clientes_saldo 
                SET STAMOV = 'N'
                WHERE IDEMOV = :idemov";

      $stmt_saldo = $pdo->prepare($sql_saldo);
      $stmt_saldo->bindParam(':idemov', $vsIdeMov, PDO::PARAM_INT);

      if (!$stmt->execute()) {
         http_response_code(500);
         echo json_encode(['mensagem' => 'Erro ao cancelar o saque no banco de dados[Solicitacao]']);
         exit;
      }

      if (!$stmt_saldo->execute()) {
         http_response_code(500);
         echo json_encode(['mensagem' => 'Erro ao cancelar o saque no banco de dados[Saldos]']);
         exit;
      }


      http_response_code(200);
      echo json_encode(['mensagem' => 'Solicitação de saque cancelada com sucesso!']);
