...
if (isset($resposta['IDECLI'], $resposta['TOKEN'], $_SESSION['token']) && $_SESSION['token'] === $resposta['TOKEN']) {
    ...
    if ($resposta['VLRPGO'] > obterSaldoReais($resposta['IDECLI'])){
        $response = ['mensagem' => 'Saldo Insuficiente para esta Operação'];
    ...
    $vsStaMov         = 'A';
    $vfSaldo_Reais    = ($resposta['VLRPGO'] * -1);
    ...
    if ($resposta['CARTEIRA'] == 'Simple') {
        $vfSaldo_Simple    = $resposta['QTDGRM'];
    } else if ($resposta['CARTEIRA'] == 'Classic'){
        $vfSaldo_Classic   = $resposta['QTDGRM'];
    } else if ($resposta['CARTEIRA'] == 'Standard'){
        $vfSaldo_Standard  = $resposta['QTDGRM'];
    } else if ($resposta['CARTEIRA'] == 'Premium'){
        $vfSaldo_Premium   = $resposta['QTDGRM'];
    }
    ...
    try {       
        ...
        $response = ['mensagem' => 'Você comprou '. $resposta['QTDGRM'] . 'g de Ouro para a Carteira '. $resposta['CARTEIRA']];
        header('Content-Type: application/json');
        echo json_encode($response);
