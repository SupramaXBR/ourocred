...
if (isset($resposta['IDECLI'], $resposta['TOKEN'], $_SESSION['token']) && $_SESSION['token'] === $resposta['TOKEN']) {
   ...
   // REMOVIDO: A validação VLRPGO > obterSaldoReais() foi removida daqui
   // pois VLRPGO é o valor que o cliente RECEBERÁ, não um valor a ser verificado contra o saldo atual.
   ...
   $vsVlrBseClc      = (float) (RetornarValorGrama() - obterValorDescGramaVendida()); // Ajustado para melhor precisão
   $vsStaMov         = 'A';
   $vfSaldo_Reais    = $resposta['VLRPGO']; // Este é o valor que entrará em reais

   $vfSaldo_Simple   = 0.00;
   $vfSaldo_Classic  = 0.00;
   $vfSaldo_Standard = 0.00;
   $vfSaldo_Premium  = 0.00;
   $vsCarteira       = $resposta['CARTEIRA'];
   $vsUsrIns         = 'User';

   if ($resposta['QTDGRM'] < 1) {
      header('Content-Type: application/json', true, 500);
      echo json_encode(['mensagem' => 'Você não pode vender menos que 1g de Ouro']);
      exit;
   }

   if ($resposta['CARTEIRA'] == 'Simple') {
      $vfSaldo_Simple    = ($resposta['QTDGRM'] * -1);
      if ($resposta['QTDGRM'] > obterSaldoSimple($vsIdeCli)) {
         header('Content-Type: application/json', true, 500);
         echo json_encode(['mensagem' => 'Voce não tem Ouro Suficiente na Carteira Simple']);
         exit;
      }
   } else if ($resposta['CARTEIRA'] == 'Classic') {
      $vfSaldo_Classic   = ($resposta['QTDGRM'] * -1);
      if ($resposta['QTDGRM'] > obterSaldoClassic($vsIdeCli)) {
         header('Content-Type: application/json', true, 500);
         echo json_encode(['mensagem' => 'Voce não tem Ouro Suficiente na Carteira Classic']);
         exit;
      }

      // Pega a última data de compra
      $dataUltimaCompra = retornarDataUltimaCompra($vsIdeCli, 'Classic');
      // Pega os dias de bloqueio da empresa
      $qtdDiasBloqueio = retornarQtddiaClassic($vsCodEmp);
      // Verifica se a data da última compra ainda está dentro do período de bloqueio
      if ($dataUltimaCompra !== null) {
         $dataDesbloqueio = date('Y-m-d H:i:s', strtotime($dataUltimaCompra . " +{$qtdDiasBloqueio} days"));
         $dataAtual = date('Y-m-d H:i:s');
         if ($dataAtual < $dataDesbloqueio) {
            // Ainda está bloqueada para venda
            header('Content-Type: application/json', true, 500);
            echo json_encode(['mensagem' => "Carteira Classic ainda está bloqueada para venda até: " . date('d/m/Y H:i:s', strtotime($dataDesbloqueio))]);
            exit;
         }
      }
   } else if ($resposta['CARTEIRA'] == 'Standard') {
      $vfSaldo_Standard  = ($resposta['QTDGRM'] * -1);
      if ($resposta['QTDGRM'] > obterSaldoStandard($vsIdeCli)) {
         header('Content-Type: application/json', true, 500);
         echo json_encode(['mensagem' => 'Voce não tem Ouro Suficiente na Carteira Standard']);
         exit;
      }
      // Pega a última data de compra
      $dataUltimaCompra = retornarDataUltimaCompra($vsIdeCli, 'Standard');
      // Pega os dias de bloqueio da empresa
      $qtdDiasBloqueio = retornarQtddiaStandard($vsCodEmp);
      // Verifica se a data da última compra ainda está dentro do período de bloqueio
      if ($dataUltimaCompra !== null) {
         $dataDesbloqueio = date('Y-m-d H:i:s', strtotime($dataUltimaCompra . " +{$qtdDiasBloqueio} days"));
         $dataAtual = date('Y-m-d H:i:s');
         if ($dataAtual < $dataDesbloqueio) {
            // Ainda está bloqueada para venda
            header('Content-Type: application/json', true, 500);
            echo json_encode(['mensagem' => "Carteira Standard ainda está bloqueada para venda até: " . date('d/m/Y H:i:s', strtotime($dataDesbloqueio))]);
            exit;
         }
      }
   } else if ($resposta['CARTEIRA'] == 'Premium') {
      $vfSaldo_Premium   = ($resposta['QTDGRM'] * -1);
      if ($resposta['QTDGRM'] > obterSaldoPremium($vsIdeCli)) {
         header('Content-Type: application/json', true, 500);
         echo json_encode(['mensagem' => 'Voce não tem Ouro Suficiente na Carteira Premium']);
         exit;
      }
      // Pega a última data de compra
      $dataUltimaCompra = retornarDataUltimaCompra($vsIdeCli, 'Premium');
      // Pega os dias de bloqueio da empresa
      $qtdDiasBloqueio = retornarQtddiaPremium($vsCodEmp);
      // Verifica se a data da última compra ainda está dentro do período de bloqueio
      if ($dataUltimaCompra !== null) {
         $dataDesbloqueio = date('Y-m-d H:i:s', strtotime($dataUltimaCompra . " +{$qtdDiasBloqueio} days"));
         $dataAtual = date('Y-m-d H:i:s');
         if ($dataAtual < $dataDesbloqueio) {
            // Ainda está bloqueada para venda
            header('Content-Type: application/json', true, 500);
            echo json_encode(['mensagem' => "Carteira Premium ainda está bloqueada para venda até: " . date('d/m/Y H:i:s', strtotime($dataDesbloqueio))]);
            exit;
         }
      }
   }
   ...
   try {
      ...
      $response = ['mensagem' => 'Você vendeu ' . $resposta['QTDGRM'] . 'g de Ouro da Carteira ' . $resposta['CARTEIRA']];
